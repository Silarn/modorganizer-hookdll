...
Disasm::Disasm() throw() : _cmd_info(), _start(nullptr), _cur(nullptr), _64bit(false) {}

Disasm::Disasm(PBYTE code, bool e64bit) : _cmd_info(), _start(nullptr), _cur(nullptr) { Init(code, e64bit); }
...
BOOL Disasm::IsGroup(short group, short groupMember) const throw() {
    return _cmd_info.isGroup && (_cmd_info.group == group) && (_cmd_info.groupMember == groupMember);
}

int Disasm::GetOpType1() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op1type != OPT_CGROUP)) {
...
int Disasm::GetOpType2() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op2type != OPT_CGROUP)) {
...
int Disasm::GetOpType3() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op3type != OPT_CGROUP)) {
...
int Disasm::GetOpSize1() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op1size != OPS_CGROUP)) {
...
int Disasm::GetOpSize2() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op2size != OPS_CGROUP)) {
...
int Disasm::GetOpSize3() throw() {
    const TableEntry* table = _cmd_info.twoByteOpcode ? table2 : table1;
    if ((_cmd_info.isGroup) && (groups[_cmd_info.group][_cmd_info.groupMember].op3size != OPS_CGROUP)) {
...
PBYTE Disasm::GetAbsoluteDestination() {
    ...
    // this is broken! size for "byteorword" and "wordordword" depend on protected vs. real-mode, 32bit vs. 64bit and
    // segment information. argh!
    switch (GetOpSize1()) {
    case OPS_BYTE: {
...
PBYTE Disasm::CopyRelative(PBYTE target, PBYTE pos1, size_t size) {
    ...
    if ((absoluteDestination >= pos1) && (absoluteDestination < pos1 + size)) {
...
const char* Disasm::GetName() const throw() { return _cmd_info.name; }

BYTE Disasm::GetOpcode() const throw() { return _cmd_info.opcode; }

BOOL Disasm::TwoByteOp() const throw() { return _cmd_info.twoByteOpcode; }

BYTE Disasm::GetReg1() const { return _cmd_info.modrm & 0x07; }

BYTE Disasm::GetReg2() const { return (_cmd_info.modrm & 0x38) >> 3; }

size_t Disasm::GetSize() const throw() { return _cmd_info.numBytes; }

BOOL Disasm::IsRelative() const throw() { return _cmd_info.relative; }

void Disasm::ResetCmdInfo() throw() { memset(&_cmd_info, 0, sizeof(_cmd_info)); }
...
size_t Disasm::GetImmediateBytes(int optype, int opsize, bool oso) throw() {
    if ((optype == OPT_IMMEDIATE) || (optype == OPT_ADDR) || (optype == OPT_RELOFFSET) || (optype == OPT_OFFSETONLY)) {
        switch (opsize) {
        case OPS_BYTE: {
            return 1;
        } break;
...
void Disasm::ReadCommand(BYTE* pos) {
    ...
    { // read prefix bytes
        int numPrefixes = 0;
        while ((table1[*curByte].flags & PREFIX) || (_64bit && ((*curByte >= 0x40) && (*curByte <= 0x4F)))) {
    ...
    { // read opcode
        ...
        if (((*curByte == 0x38) || (*curByte == 0x3A)) && (_cmd_info.twoByteOpcode)) {
            // TODO: fix this!
            return;
        }
    ...
    if (HasSIB(_cmd_info.modrm)) {
        ...
        if ((_cmd_info.sib & 0x07) == 0x05) {
            // sib specifies
            if (((_cmd_info.modrm & 0xC0) == 0x00) || ((_cmd_info.modrm & 0xC0) == 0x80)) {
                displacement = 4;
            } else if ((_cmd_info.modrm & 0xC0) == 0x40) {
                displacement = 1;
            }
...
const PBYTE Disasm::GetDispl() throw() { return _cmd_info.displ; }

const PBYTE Disasm::GetParam1() throw() { return _cmd_info.param1; }

FuncDisasm::FuncDisasm() throw() : _start(nullptr), _end(nullptr) {}

FuncDisasm::FuncDisasm(PBYTE code, bool e64bit) { Init(code, e64bit); }
...
void FuncDisasm::Init(PBYTE code, bool e64bit) {
    ...
    do {
        ...
        if ((!_disasm.TwoByteOp() && ((opcode == 0xC3) ||                                       // RET
                                      (opcode == 0xCB) ||                                       // RET
                                      (opcode == 0xC2) ||                                       // RET
                                      (opcode == 0xCA) ||                                       // RET
                                      (opcode == 0xE9) ||                                       // JMP near relative
                                      (opcode == 0xEA) ||                                       // JMP far absolute
                                      ((opcode == 0xCD) && (*_disasm.GetParam1() == 0x2E)))) || // INT 2E
