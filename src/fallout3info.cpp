...
Fallout3Info::Fallout3Info(const std::wstring& gameDirectory) : GameInfo(gameDirectory) {
    identifyMyGamesDirectory(L"fallout3");
}

bool Fallout3Info::identifyGame(const std::wstring& searchPath) {
    return FileExists(searchPath, L"Fallout3.exe") && FileExists(searchPath, L"FalloutLauncher.exe");
}

std::wstring Fallout3Info::getRegPathStatic() {
    HKEY key;
    LONG errorcode =
        ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Software\\Bethesda Softworks\\Fallout3", 0, KEY_QUERY_VALUE, &key);
...
std::vector<std::wstring> Fallout3Info::getIniFileNames() const { return {L"fallout.ini", L"falloutprefs.ini"}; }

std::wstring Fallout3Info::getReferenceDataFile() const { return L"Fallout - Meshes.bsa"; }
