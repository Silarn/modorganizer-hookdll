...
class ModInfo {
  ...
  public:
    ModInfo(const std::wstring& profileName, bool enableHiding, const std::wstring& moPath,
    ...
    HANDLE findStart(LPCWSTR lpFileName, FINDEX_INFO_LEVELS fInfoLevelId, LPVOID lpFindFileData,
    ...
    std::string getRerouteOpenExisting(LPCSTR originalName, bool preferOriginal = false, bool* rerouted = nullptr,
                                       int* originID = nullptr);
    std::wstring getRerouteOpenExisting(LPCWSTR originalName, bool preferOriginal = false, bool* rerouted = nullptr,
                                        int* originID = nullptr);
  ...
  private:
    ...
    void addSearchResults(SearchBuffer& searchBuffer, HANDLE& primaryHandle, HANDLE searchHandle, LPCWSTR directory,
    ...
    HANDLE dataSearch(LPCWSTR absoluteFileName, size_t filenameOffset, HANDLE dataHandle, WIN32_FIND_DATAW& tempData,
                      FINDEX_INFO_LEVELS fInfoLevelId, FINDEX_SEARCH_OPS fSearchOp, LPVOID lpSearchFilter,
                      DWORD dwAdditionalFlags);

    void loadDeleters(const std::string& fileName);

    void createDirectoryStructure(MOShared::DirectoryEntry& directoryEntry, const std::wstring& basePath,
                                  const std::wstring& dirName, int index);
  ...
  private:
    struct RemovalInfo {
        ...
        RemovalInfo(const RemovalInfo& reference)
            : fileName(reference.fileName), origin(reference.origin), time(reference.time) {}
