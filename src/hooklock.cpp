...
class __tlsCleanup {
  public:
    __tlsCleanup() { m_Index = ::TlsAlloc(); }
    ~__tlsCleanup() { ::TlsFree(m_Index); }

    DWORD index() { return m_Index; }
...
HookLock::HookLock(DWORD group) : m_Owner(false), m_Group(group) {
...
bool HookLock::isLocked(DWORD group) { return (GetTLS() & group) != 0UL; }
