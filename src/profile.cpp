    ...
    if (!s_Times.empty()) {
        ...
        for (std::map<const char*, Time>::const_iterator iter = s_Times.begin(); iter != s_Times.end(); ++iter) {
            Logger::Instance().info(
                "%s: %lu calls, %d to %d us (total: %d us, avg: %d us)", iter->first, iter->second.m_Count,
                static_cast<int>(iter->second.m_Min * 1000000), static_cast<int>(iter->second.m_Max * 1000000),
