...
BOOL WINAPI CreateProcessA_rep(LPCSTR lpApplicationName, LPSTR lpCommandLine, LPSECURITY_ATTRIBUTES lpProcessAttributes,
    ...
    LOGDEBUG("create process (a) %s - %s (in %s) - %s", lpApplicationName != nullptr ? lpApplicationName : "null",
             lpCommandLine != nullptr ? lpCommandLine : "null",
             lpCurrentDirectory != nullptr ? lpCurrentDirectory : "null", blacklisted ? "NOT hooking" : "hooking");
    ...
    if (!::CreateProcessA_reroute(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes,
                                  bInheritHandles, flags, lpEnvironment,
                                  lpCurrentDirectory != nullptr ? reroutedCwd.c_str() : nullptr, lpStartupInfo,
    ...
    try {
        if (!blacklisted) {
            ...
            injectDLL(lpProcessInformation->hProcess, lpProcessInformation->hThread, hookPath,
                      modInfo->getProfileName(), sLogLevel);
        }
...
BOOL WINAPI CreateProcessW_rep(LPCWSTR lpApplicationName, LPWSTR lpCommandLine,
    ...
    LOGDEBUG("create process (w) %ls - %ls (in %ls) - %s", lpApplicationName != nullptr ? lpApplicationName : L"null",
             lpCommandLine != nullptr ? lpCommandLine : L"null",
             lpCurrentDirectory != nullptr ? lpCurrentDirectory : L"null", blacklisted ? "NOT hooking" : "hooking");
    ...
    if (!::CreateProcessW_reroute(lpApplicationName, lpCommandLine, lpProcessAttributes, lpThreadAttributes,
                                  bInheritHandles, flags, lpEnvironment,
                                  lpCurrentDirectory != nullptr ? reroutedCwd.c_str() : nullptr, lpStartupInfo,
                                  lpProcessInformation)) {
        LOGDEBUG("process failed to start (%lu)", ::GetLastError());
...
HANDLE WINAPI CreateFileW_rep(LPCWSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
    ...
    if (((dwCreationDisposition == CREATE_ALWAYS) || (dwCreationDisposition == CREATE_NEW) ||
        ...
        // need to check if the file exists. If it does, act on the existing file, otherwise the behaviour is not
        // transparent if the regular call causes an error message and rerouted to overwrite
    ...
    if (rerouteFilename.length() == 0) {
        ...
        if (bsaName != bsaMap.end()) {
            ...
            if (!rerouted) {
                LOGDEBUG("createfile bsa not rerouted: %ls -> %ls -> %ls", lpFileName, bsaPath.c_str(),
                         rerouteFilename.c_str());
            }
            // bsa found under its obfuscated name, don't let the game find it under its original name any more,
            // otherwise that would overwrite the load order again
    ...
    HANDLE result = CreateFileW_reroute(rerouteFilename.c_str(), dwDesiredAccess, dwShareMode, lpSecurityAttributes,
                                        dwCreationDisposition, dwFlagsAndAttributes, hTemplateFile);

    if (PathStartsWith(fullFileName, modInfo->getDataPathW().c_str()) && (result != INVALID_HANDLE_VALUE) &&
    ...
    if (rerouted) {
        LOGDEBUG("createfile w: %ls -> %ls (%x - %x) = %p (%d)", lpFileName, rerouteFilename.c_str(), dwDesiredAccess,
                 dwCreationDisposition, result, ::GetLastError());
    }

    return result;
}

HANDLE WINAPI CreateFileA_rep(LPCSTR lpFileName, DWORD dwDesiredAccess, DWORD dwShareMode,
                              LPSECURITY_ATTRIBUTES lpSecurityAttributes, DWORD dwCreationDisposition,
                              DWORD dwFlagsAndAttributes, HANDLE hTemplateFile) {
    PROFILE();
...
DWORD WINAPI GetFileAttributesW_rep(LPCWSTR lpFileName) {
    ...
    std::map<std::string, std::string>::iterator bsaName = bsaMap.find(ToString(baseName, true));
    if (bsaName != bsaMap.end()) {
        rerouteFilename = modInfo->getRerouteOpenExisting(
            (std::wstring(lpFileName).substr(0, pathLen) + ToWString(bsaName->second, true)).c_str(), false, &rerouted);
    } else {
        rerouteFilename = modInfo->getRerouteOpenExisting(lpFileName, false, &rerouted);
    }
...
BOOL WINAPI GetFileAttributesExW_rep(LPCWSTR lpFileName, GET_FILEEX_INFO_LEVELS fInfoLevelId,
    ...
    if (bsaName != bsaMap.end()) {
        rerouteFilename = modInfo->getRerouteOpenExisting(
            std::wstring(lpFileName).substr(0, pathLen).append(ToWString(bsaName->second, true)).c_str(), false,
            &rerouted);
    } else {
        rerouteFilename = modInfo->getRerouteOpenExisting(lpFileName, false, &rerouted);
    }
    ...
    /*  if (rerouted) {
        if (result && (fInfoLevelId == GetFileExInfoStandard)) {
          ...
          LOGDEBUG("get file attributesex: %ls -> %ls: %d (%d)", lpFileName, rerouteFilename.c_str(), result,
      fileData->dwFileAttributes); } else { LOGDEBUG("get file attributesex: %ls -> %ls: %d", lpFileName,
      rerouteFilename.c_str(), result);
        }
      }*/
    return result;
}
...
HANDLE WINAPI FindFirstFileExW_rep(LPCWSTR lpFileName, FINDEX_INFO_LEVELS fInfoLevelId, LPVOID lpFindFileData,
                                   FINDEX_SEARCH_OPS fSearchOp, LPVOID lpSearchFilter, DWORD dwAdditionalFlags) {
    PROFILE();

    if (HookLock::isLocked(HookLock::FIND_FILE_GROUP) || (lpFileName == nullptr)) {
        return FindFirstFileExW_reroute(lpFileName, fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter,
                                        dwAdditionalFlags);
    }
    ...
    bool rerouted = false;
    HANDLE result = modInfo->findStart(rerouteFilename.c_str(), fInfoLevelId, lpFindFileData, fSearchOp, lpSearchFilter,
                                       dwAdditionalFlags, &rerouted);
...
BOOL WINAPI DeleteFileW_rep(LPCWSTR lpFileName) {
    ...
    if (PathStartsWith(buffer, modInfo->getDataPathW().c_str())) {
        ...
        std::wstring rerouteFilename =
            modInfo->getProfilePath().append(L"\\saves\\").append(sPos + wcslen(AppConfig::localSavePlaceholder()));
        modInfo->removeModFile(lpFileName);
        Logger::Instance().info("deleting %ls -> %ls", lpFileName, rerouteFilename.c_str());
        return DeleteFileW_reroute(rerouteFilename.c_str());
    } else {
...
BOOL WINAPI MoveFileExW_rep(LPCWSTR lpExistingFileName, LPCWSTR lpNewFileName, DWORD dwFlags) {
    ...
    if (PathStartsWith(fullDestinationName, modInfo->getDataPathW().c_str())) {
        ...
        // usually, always move to the overwrite directory. However, in the "create tmp, remove original, move tmp to
        // original"-sequence we'd rather have the modified file in the original location. If the source file was part
        // of a mod we leave the file in that mod
        std::wostringstream temp;
        if (!destinationReroute.empty()) {
            // In the "create tmp, remove original, move tmp to original"-sequence we'd rather have the modified file in
        ...
        destinationReroute =
            modInfo->getProfilePath() + L"\\saves\\" + (sPos + wcslen(AppConfig::localSavePlaceholder()));
...
BOOL WINAPI MoveFileA_rep(LPCSTR lpExistingFileName, LPCSTR lpNewFileName) {
    ...
    return MoveFileW_rep(ToWString(lpExistingFileName, false).c_str(), ToWString(lpNewFileName, false).c_str());
}

BOOL WINAPI MoveFileExA_rep(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, DWORD dwFlags) {
    PROFILE();

    return MoveFileExW_rep(ToWString(lpExistingFileName, false).c_str(), ToWString(lpNewFileName, false).c_str(),
                           dwFlags);
}
...
#define NOPEMIT(n) __asm _emit 0x90
...
static bool identifyAndManipulate(DWORD* pos, DWORD size) {
    ...
    enum Registers { REG_EAX, REG_EBX, REG_ECX, REG_EDX } resultRegister = REG_ECX;
    ...
    *reinterpret_cast<DWORD*>(pushInst + 1) = size;
    ...
    case 0:
        function = reinterpret_cast<void*>(pushModEAX);
        break;
    case 1:
        function = reinterpret_cast<void*>(pushModECX);
        break;
    case 2:
        function = reinterpret_cast<void*>(pushModEDX);
        break;
    case 3:
        function = reinterpret_cast<void*>(pushModEBX);
        break;
    default:
        return false;
    }
    ...
    case REG_EAX:
        function = reinterpret_cast<void*>(returnInstrEAX);
        break;
    case REG_EBX:
        function = reinterpret_cast<void*>(returnInstrEBX);
        break;
    case REG_ECX:
        function = reinterpret_cast<void*>(returnInstrECX);
        break;
    case REG_EDX:
        function = reinterpret_cast<void*>(returnInstrEDX);
        break;
    }
    ...
    ULONG distance = reinterpret_cast<ULONG>(s_FunctionBuffer) - 1 - (reinterpret_cast<ULONG>(funcPtr) + sizeof(ULONG));
...
DWORD WINAPI GetPrivateProfileStringA_rep(LPCSTR lpAppName, LPCSTR lpKeyName, LPCSTR lpDefault, LPSTR lpReturnedString,
    ...
    { // mod-inis are used directly
        ...
        if (iniFilesA.find(fileName) == iniFilesA.end()) {
            ...
            DWORD res = GetPrivateProfileStringA_reroute(lpAppName, lpKeyName, lpDefault, lpReturnedString, nSize,
                                                         rerouteFilename.c_str());
    ...
    if ((archiveListHookState == HOOK_NOTYET) && (lpKeyName != nullptr) && (lpKeyName[0] == 's') &&
    ...
    if ((lpKeyName != nullptr) &&
        ...
        if ((length > 255) && (lpReturnedString != s_Buffer)) {
            LOGDEBUG("safety check: length exceeds regular size but wrong buffer (%p vs. %p)?", lpReturnedString,
                     s_Buffer);
            length = 255;
        }
        ...
        if (strcmp(temp.get(), "DUMMY_VALUE") == 0) {
            ...
            res = GetPrivateProfileStringA_reroute(lpAppName, lpKeyName, lpDefault, temp.get(), nSize,
                                                   rerouteFilename.c_str());
...
DWORD WINAPI GetPrivateProfileStringW_rep(LPCWSTR lpAppName, LPCWSTR lpKeyName, LPCWSTR lpDefault,
    ...
    if (lpFileName != nullptr) {
        DWORD res = GetPrivateProfileStringW_reroute(lpAppName, lpKeyName, L"DUMMY_VALUE", lpReturnedString, nSize,
                                                     modInfo->getTweakedIniW().c_str());
        if (wcscmp(lpReturnedString, L"DUMMY_VALUE") == 0) {
            std::wstring rerouteFilename = modInfo->getRerouteOpenExisting(lpFileName);
            res = GetPrivateProfileStringW_reroute(lpAppName, lpKeyName, lpDefault, lpReturnedString, nSize,
                                                   rerouteFilename.c_str());
        } else if (lpKeyName != nullptr) {
...
BOOL WINAPI GetPrivateProfileStructA_rep(LPCSTR lpszSection, LPCSTR lpszKey, LPVOID lpStruct, UINT uSizeStruct,
                                         LPCSTR szFile) {
    PROFILE();
...
BOOL WINAPI GetPrivateProfileStructW_rep(LPCWSTR lpszSection, LPCWSTR lpszKey, LPVOID lpStruct, UINT uSizeStruct,
                                         LPCWSTR szFile) {
    PROFILE();
...
DWORD WINAPI GetPrivateProfileSectionW_rep(LPCWSTR lpAppName, LPWSTR lpReturnedString, DWORD nSize,
                                           LPCWSTR lpFileName) {
    PROFILE();
...
UINT WINAPI GetPrivateProfileIntW_rep(LPCWSTR lpAppName, LPCWSTR lpKeyName, INT nDefault, LPCWSTR lpFileName) {
    ...
    HookLock lock(HookLock::GET_PROFILESTRING_GROUP); // on some (all?) systems, getprivateprofileint calls
...
// ensures that the specified file exists (virtually) IF it is inside the data directory
void MakeFileExist(LPCSTR fileName) { MakeFileExist(ToWString(fileName, false).c_str()); }
    ...
    if (lpKeyName != nullptr) {
        ...
        if (tweakedIniValues.find(keyW) != tweakedIniValues.end()) {
            if (ToWString(lpString, false) != tweakedIniValues[keyW]) {
                ...
                BOOL res = WritePrivateProfileStringA_reroute(lpAppName, lpKeyName, lpString,
                                                              modInfo->getTweakedIniA().c_str());
                // also store in "profile_tweaks.ini" for this profile so the settings can be applied in the future
                WritePrivateProfileStringA_reroute(
...
BOOL WINAPI WritePrivateProfileStructA_rep(LPCSTR lpszSection, LPCSTR lpszKey, LPVOID lpStruct, UINT uSizeStruct,
                                           LPCSTR szFile) {
    PROFILE();
...
BOOL WINAPI WritePrivateProfileStructW_rep(LPCWSTR lpszSection, LPCWSTR lpszKey, LPVOID lpStruct, UINT uSizeStruct,
                                           LPCWSTR szFile) {
    PROFILE();
...
BOOL WINAPI CopyFileA_rep(LPCSTR lpExistingFileName, LPCSTR lpNewFileName, BOOL bFailIfExists) {
    ...
    return CopyFileW_rep(ToWString(lpExistingFileName, false).c_str(), ToWString(lpNewFileName, false).c_str(),
                         bFailIfExists);
}
...
BOOL WINAPI CreateHardLinkW_rep(LPCWSTR lpFileName, LPCWSTR lpExistingFileName,
    ...
    if ((sourceID == destID) && (sourceID != -1) && (wcsncmp(fsName, L"NTFS", 10) == 0)) {
        ...
        LOGDEBUG("copy file (link impossible): %ls -> %ls", rerouteExistingFileName.c_str(),
                 rerouteNewFileName.c_str());
...
BOOL WINAPI CreateHardLinkA_rep(LPCSTR lpFileName, LPCSTR lpExistingFileName,
    ...
    return CreateHardLinkW_rep(ToWString(lpFileName, false).c_str(), ToWString(lpExistingFileName, false).c_str(),
...
DWORD WINAPI GetFullPathNameW_rep(LPCWSTR lpFileName, DWORD nBufferLength, LPWSTR lpBuffer, LPWSTR* lpFilePart) {
    ...
    if ((wcslen(searchPath) > 2) && (searchPath[1] == L':') && (searchPath[2] == L'.')) {
        searchPath += 2;
    }
    ...
    if (modInfo->getCurrentDirectory().length() != 0) {
        ...
        if (StartsWith(lpFileName, cwd)) {
            WCHAR temp[MAX_PATH];
            ::PathCombineW(temp, modInfo->getCurrentDirectory().c_str(),
                           lpFileName + static_cast<size_t>(cwdLength) + 1);
...
int STDAPICALLTYPE SHFileOperationA_rep(LPSHFILEOPSTRUCTA lpFileOp) {
        ...
        for (;;) {
            ...
            if (!rerouted && PathStartsWith(pos, modInfo->getDataPathA().c_str())) {
                ...
                if (strlen(pos) == modInfo->getDataPathA().length()) {
                    ...
                    temp << ToString(modInfo->getOverwritePath(), false) << "\\"
                         << (pos + modInfo->getDataPathA().length());
                    rerouteFilename = temp.str();
    ...
    LOGDEBUG("sh file operation a %d: %s - %s", newOp.wFunc, newOp.pFrom, newOp.pTo != nullptr ? newOp.pTo : "nullptr");
...
BOOL WINAPI GetFileVersionInfoW_rep(LPCWSTR lptstrFilename, DWORD dwHandle, DWORD dwLen, LPVOID lpData) {
    ...
    HookLock lock(HookLock::GET_FILEVERSION_GROUP); // GetFileVersionInfoW calls the ex variant on some but not all
                                                    // windows variants
...
BOOL WINAPI GetFileVersionInfoExW_rep(DWORD dwFlags, LPCWSTR lptstrFilename, DWORD dwHandle, DWORD dwLen,
                                      LPVOID lpData) {
    PROFILE();
...
DWORD WINAPI GetFileVersionInfoSizeW_rep(LPCWSTR lptstrFilename, LPDWORD lpdwHandle) {
    ...
    if (rerouted) {
        LOGDEBUG("get file version size %ls -> %ls -> %lu (%x)", lptstrFilename, rerouteFilename.c_str(), res,
                 ::GetLastError());
    }
...
NTSTATUS addNtSearchData(const std::wstring& localPath, PUNICODE_STRING FileName,
    ...
    HANDLE hdl = ::CreateFileW_reroute(localPath.c_str(), GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, nullptr,
...
// case where, on first call, the buffer is allowed to be smaller than the data element as long as it's long enough to
// receive the fixed block size. It doesn't signal IO Completion objects. It ... augh fuck this function is
// complicated...
NTSTATUS WINAPI NtQueryDirectoryFile_rep(HANDLE FileHandle, HANDLE Event, PIO_APC_ROUTINE ApcRoutine, PVOID ApcContext,
    ...
    if ((directoryCFHandles.find(FileHandle) != directoryCFHandles.end()) &&
        ...
        LOGDEBUG("ntquerydirectoryfile called with a rerouted handle from CreateFile (%d) (%d) (%.*ls)",
                 ReturnSingleEntry, FileInformationClass, FileName != nullptr ? FileName->Length : 4,
                 FileName != nullptr ? FileName->Buffer : L"null");
        if (RestartScan) {
        ...
        if (qdfData.find(FileHandle) == qdfData.end()) {
            ...
            std::wstring relativePath =
                directoryCFHandles[FileHandle].size() > modInfo->getDataPathW().size()
            ...
            for (auto iter = modNames.begin(); iter != modNames.end(); ++iter) {
                ...
                if (addNtSearchData(localPath, FileName, FileInformationClass, buffer, bufferSize, result,
                                    foundFiles) != STATUS_NO_SUCH_FILE) {
                    noSuchFile = false;
                }
            }
            queryMutex.lock();
        ...
        while (qdfData[FileHandle].size() > 0) {
            ...
            if (data.size() < (Length - offset)) {
                ...
                offset += (std::max<int>)(size, data.size()); // size is >= data.size except for the last element, so
                                                              // that after this loop offset contains the total data
                                                              // copied
...
#define INITHOOK(module, functionname)                                                                                 \
    {                                                                                                                  \
        ApiHook* temp = new ApiHook(module, #functionname, (void*)&functionname##_rep);                                \
...
BOOL SetUp(const std::wstring& iniName, const wchar_t* profileNameIn, const std::wstring& moPath,
           const std::wstring& moDataPath) {
        ...
        if (funcDump) {
            ...
            if (::GetModuleFileNameW(dllModule, dmpPath, MAX_PATH_UNICODE) == 0) {
                Logger::Instance().error(
                    "No crash dump created, failed to determine destination directory. errorcode: %lu",
                    ::GetLastError());
                return;
            }
            wcscat(dmpPath, L".dmp");

            HANDLE dumpFile = ::CreateFileW_reroute(dmpPath, GENERIC_WRITE, FILE_SHARE_WRITE, nullptr, CREATE_ALWAYS,
            ...
            if (dumpFile != INVALID_HANDLE_VALUE) {
                ...
                if (success) {
                    Logger::Instance().error("Crash dump created as %ls. Please send this file to the developer of MO",
                                             dmpPath);
                } else {
...
LONG WINAPI VEHandler(PEXCEPTION_POINTERS exceptionPtrs) {
    ...
    if (((DWORD)exceptionPtrs->ExceptionRecord->ExceptionAddress < start) ||
        ...
        if ((exceptionPtrs->ExceptionRecord->ExceptionCode >= 0x80000000)    // only "warnings"
            ...
            Logger::Instance().info("Windows Exception (%x). Origin: \"%ls\" (%x). Last hooked call: %s",
                                    exceptionPtrs->ExceptionRecord->ExceptionCode, modName.c_str(),
                                    exceptionPtrs->ExceptionRecord->ExceptionAddress, s_LastFunction);
        ...
        if ((exceptionPtrs->ExceptionRecord->ExceptionFlags != EXCEPTION_NONCONTINUABLE) ||
            ...
            // don't want to break on non-critical exceptions. 0xe06d7363 indicates a C++ exception. why are those
            // marked non-continuable?
...
bool Init(int logLevel, const wchar_t* profileName) {
    ...
    Logger::Instance().info("Windows %d.%d (%s)", versionInfo.dwMajorVersion, versionInfo.dwMinorVersion,
                            versionInfo.wProductType == VER_NT_WORKSTATION ? "workstation" : "server");
    ::GetModuleFileNameW(dllModule, pathBuffer, MAX_PATH_UNICODE);
    VS_FIXEDFILEINFO version = GetFileVersion(pathBuffer);
    Logger::Instance().info("hook.dll v%d.%d.%d", version.dwFileVersionMS >> 16, version.dwFileVersionMS & 0xFFFF,
