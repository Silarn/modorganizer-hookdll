...
GameInfo::GameInfo(const std::wstring& gameDirectory) : m_GameDirectory(gameDirectory) { atexit(&cleanup); }
...
std::wstring GameInfo::getGameDirectory() const { return m_GameDirectory; }

std::wstring GameInfo::getSpecialPath(LPCWSTR name) const {
    HKEY key;
    LONG errorcode = ::RegOpenKeyExW(HKEY_CURRENT_USER,
                                     L"Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\User Shell Folders", 0,
                                     KEY_QUERY_VALUE, &key);
