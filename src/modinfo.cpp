...
bool FileExists_reroute(const std::wstring& filename) {
    ...
    search =
        FindFirstFileExW_reroute(filename.c_str(), FindExInfoStandard, &findData, FindExSearchNameMatch, nullptr, 0);
...
ModInfo::ModInfo(const std::wstring& profileName, bool enableHiding, const std::wstring& moPath,
                 const std::wstring& moDataPath)
    : m_ProfileName(profileName), m_CurrentDirectory(), m_MOPathW(moPath), m_MODataPathW(moDataPath),
      m_DirectoryStructure(L"data", nullptr, 0), m_ModCount(0), m_SavesReroute(false) {
...

    {
        ...
        ::GetPrivateProfileStringW(L"Settings", L"mod_directory",
                                   (m_MODataPathW + L"\\" + AppConfig::modsPath()).c_str(), temp, MAX_PATH,
                                   (m_MODataPathW + L"\\" + AppConfig::iniFileName()).c_str());
    ...
    HANDLE dataDir = ::CreateFileW(m_DataPathAbsoluteW.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING,
    ...
    { // see if there is a potential alternative game path
        ...
        GetFinalPathNameByHandleW_type getFinalPathNameByHandleW =
            (GetFinalPathNameByHandleW_type)::GetProcAddress(kernel32Handle, "GetFinalPathNameByHandleW");
        ...
        if (m_DataPathAbsoluteAlternativeW.length() == 0) {
            ...
            if (!regPath.empty()) {
                ...
                if (!PathStartsWith(m_DataPathAbsoluteW.c_str(), regPath.c_str())) {
                    ...
                    Logger::Instance().info("data path from registry differs from configured game path: %ls",
                                            regPath.c_str());
                }
            }
        }
    ...
    for (std::vector<std::wstring>::reverse_iterator modIter = m_ModList.rbegin(); modIter != m_ModList.rend();
         ++modIter, ++index) {
        ...
        if (modIter->at(0) == L'*') {
            ...
            m_UpdateHandles.push_back(
                ::FindFirstChangeNotificationW(modPath.c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME));
        ...
        while (success) {
            ...
            m_DirectoryStructure.addFromBSA(modName, modPath, modPath + L"\\" + findData.cFileName, index);
    ...
    m_UpdateHandles.push_back(
        ::FindFirstChangeNotificationW(m_OverwritePathW.c_str(), TRUE, FILE_NOTIFY_CHANGE_FILE_NAME));
...
const FilesOrigin& ModInfo::getFilesOrigin(int originID) const { return m_DirectoryStructure.getOriginByID(originID); }
...
void ModInfo::setMOPath(const std::wstring& moPath) { m_MOPathW = moPath; }
...
void ModInfo::addOverwriteFile(const std::wstring& fileName) { addModFile(L"overwrite", fileName); }
...
std::wstring ModInfo::getRemovedLocation(const std::wstring& fileName) {
    ...
    for (std::list<RemovalInfo>::const_iterator iter = m_RemovalInfo.begin(); iter != m_RemovalInfo.end();) {
        if (iter->time + 5 < now) {
            ...
            fullPath << m_DirectoryStructure.getOriginByID(iter->origin).getPath()
                     << (fileName.c_str() + m_DataPathAbsoluteW.length());
...
void ModInfo::removeModFile(const std::wstring& fileName) {
    ...
    if (PathStartsWith(fullPath, m_DataPathAbsoluteW.c_str()) && (wcslen(fullPath) != m_DataPathAbsoluteW.length())) {
        int origin = -1;
...
std::wstring ModInfo::getCurrentDirectory() { return m_CurrentDirectory; }
...
void ModInfo::addSearchResults(SearchBuffer& searchBuffer, HANDLE& primaryHandle, HANDLE searchHandle,
                               LPCWSTR directory, WIN32_FIND_DATAW& searchData) {
...
HANDLE ModInfo::dataSearch(LPCWSTR absoluteFileName, size_t filenameOffset, HANDLE dataHandle,
                           WIN32_FIND_DATAW& searchData, FINDEX_INFO_LEVELS fInfoLevelId, FINDEX_SEARCH_OPS fSearchOp,
    ...
    for (const std::wstring& name : m_ModList) {
        ...
        try {
            ...
            if ((relativePath[0] != '\0') &&
                ((searchDirectory == nullptr) || !searchDirectory->hasContentsFromOrigin(origin.getID()))) {
                continue;
            }
        ...
        ::ZeroMemory(&searchData, sizeof(WIN32_FIND_DATAW));
        dataHandle = FindFirstFileExW_reroute(fullPath.str().c_str(), fInfoLevelId, &searchData, fSearchOp,
    ...
    // and the overwrite folder too
    {
        ...
        ::ZeroMemory(&searchData, sizeof(WIN32_FIND_DATAW));
        dataHandle = FindFirstFileExW_reroute(fullPath.c_str(), fInfoLevelId, &searchData, fSearchOp, lpSearchFilter,
                                              dwAdditionalFlags);
    ...
    if ((primaryHandle == INVALID_HANDLE_VALUE) && (searchBuffer.empty())) {
...
HANDLE ModInfo::findStart(LPCWSTR lpFileName, FINDEX_INFO_LEVELS fInfoLevelId, LPVOID lpFindFileData,
                          FINDEX_SEARCH_OPS fSearchOp, LPVOID lpSearchFilter, DWORD dwAdditionalFlags, bool* rerouted) {
    WCHAR temp[MAX_PATH];
    ...
    if (file.get() != nullptr) {
        ...
        return FindFirstFileExW_reroute(file->getFullPath().c_str(), fInfoLevelId, lpFindFileData, fSearchOp,
                                        lpSearchFilter, dwAdditionalFlags);
    } else {
        WIN32_FIND_DATAW tempData;
        ::ZeroMemory(&tempData, sizeof(WIN32_FIND_DATAW));
        HANDLE searchHandle =
            FindFirstFileExW_reroute(lpFileName, fInfoLevelId, &tempData, fSearchOp, lpSearchFilter, dwAdditionalFlags);
        ...
        HANDLE handle = dataSearch(absoluteFileName, filenameOffset, searchHandle, tempData, fInfoLevelId, fSearchOp,
                                   lpSearchFilter, dwAdditionalFlags);
...
BOOL ModInfo::searchExists(HANDLE handle) { return m_Searches.find(handle) != m_Searches.end(); }

bool ModInfo::isFileHidden(const std::wstring& fileName) const {
    return m_HiddenFiles.find(fileName) != m_HiddenFiles.end();
}

const std::string& ModInfo::getTweakedIniA() const { return m_TweakedIniPathA; }

const std::wstring& ModInfo::getTweakedIniW() const { return m_TweakedIniPathW; }

std::string ModInfo::getRerouteOpenExisting(LPCSTR originalName, bool preferOriginal, bool* rerouted, int* originID) {
    return ToString(getRerouteOpenExisting(ToWString(originalName, false).c_str(), preferOriginal, rerouted, originID),
                    false);
}
...
std::wstring ModInfo::getRerouteOpenExisting(LPCWSTR originalName, bool preferOriginal, bool* rerouted, int* originID) {
    ...
    } else if (m_SavesReroute && (EndsWith(temp, L".skse") || EndsWith(temp, L".obse")) &&
    ...
    } else if (PathStartsWith(temp, m_DataPathAbsoluteW.c_str()) && (wcslen(temp) != m_DataPathAbsoluteW.length())) {
