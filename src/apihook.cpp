...
LPVOID MyGetProcAddress(HMODULE module, LPCSTR functionName) {
    ...
    PIMAGE_EXPORT_DIRECTORY exportDirectory =
        (PIMAGE_EXPORT_DIRECTORY)((DWORD)dosHeader + dataDirectory->VirtualAddress);
...
ApiHook::ApiHook(LPCTSTR moduleName, LPCSTR functionName, LPVOID replacement)
...
LPVOID ApiHook::GetReroute() { return reinterpret_cast<LPVOID>(_reroute); }
...
void ApiHook::AddrReplace(LPBYTE start, ULONG pattern, LPVOID addr, size_t size, BOOL relative_adjust) {
    if (start == nullptr)
        throw std::runtime_error("nullptr-Pointer as start-address");
    if (start == nullptr)
        throw std::runtime_error("nullptr-Pointer as replacement-address");
...
size_t ApiHook::CreateReroute(LPBYTE& hookInsertionAddress) {
    ...
    while (size < sizeof(jump)) {
        if ((size == 0) && (disasm.GetOpcode() == 0xE9)) {
            LOGDEBUG("%s seems to be hooked already: %ls", _functionName,
                     ::GetSectionName(disasm.GetAbsoluteDestination()).c_str());
            ...
            if (refDisasm.GetOpcode() == 0xE9) {
                ...
                LOGDEBUG("%s seems to be hooked already (hot-patch): %ls", _functionName,
                         ::GetSectionName(refDisasm.GetAbsoluteDestination()).c_str());
    ...
    _reroute = reinterpret_cast<LPBYTE>(::VirtualAlloc(nullptr, size + jlen + sizeof(char*) + relativeAdd,
...
BOOL ApiHook::InsertHook(LPVOID original, LPVOID replacement) {
    ...
    if (!::VirtualProtect(hookInsertionAddress, sizeof(jump), PAGE_EXECUTE_WRITECOPY, &oldprotect)) {
...
void ApiHook::RemoveHook() {
    if (_installed) {
        DWORD oldprotect, ignore;
        // Set the target function to copy on write, so we don't modify code for other processes
        if (!::VirtualProtect(_origPos, sizeof(jump), PAGE_EXECUTE_WRITECOPY, &oldprotect)) {
            throw MOShared::windows_error("failed to change virtual protection");
        }
