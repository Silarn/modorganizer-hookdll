...
OblivionInfo::OblivionInfo(const std::wstring& gameDirectory) : GameInfo(gameDirectory) {
    identifyMyGamesDirectory(L"oblivion");
}

bool OblivionInfo::identifyGame(const std::wstring& searchPath) {
    return FileExists(searchPath, L"Oblivion.exe") && FileExists(searchPath, L"OblivionLauncher.exe");
}

std::wstring OblivionInfo::getRegPathStatic() {
    HKEY key;
    LONG errorcode =
        ::RegOpenKeyEx(HKEY_LOCAL_MACHINE, L"Software\\Bethesda Softworks\\Oblivion", 0, KEY_QUERY_VALUE, &key);
...
std::wstring OblivionInfo::getReferenceDataFile() const { return L"Oblivion - Meshes.bsa"; }
